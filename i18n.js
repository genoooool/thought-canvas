...
const TRANSLATION_ROWS = [
  ...
  ['每条回答下方可手动拆解，也可在提问中说“帮我拆解”自动创建节点；模糊意图会先预览确认。', 'Under each answer, split manually, or say “break this down” in your question to create nodes automatically; fuzzy intent is previewed for confirmation.', '各回答の下で手動分解できます。また、質問で「分解して」と指定するとノードを自動作成し、曖昧な意図は確認用にプレビューします。'],
  ['暂不自动拆解', 'Keep as one answer', '今回は自動分解しない'],
  ['这条回答暂时没有形成足够清晰的多个模块，已保留为单一回答节点。', 'This answer did not form enough clear modules, so it remains a single answer node.', 'この回答は明確な複数モジュールにならなかったため、単一の回答ノードとして残しました。'],
  ['识别到拆解意图', 'Decomposition intent detected', '分解の意図を検出'],
  ['准备创建 {count} 个拆解节点', 'Ready to create {count} decomposition nodes', '{count} 個の分解ノードを作成します'],
  ['这条问题看起来包含多个分析层面。确认后会把当前回答拆成可继续追问的内容节点。', 'This question appears to contain multiple angles. After confirmation, the answer will be split into content nodes you can continue exploring.', 'この質問には複数の分析面があるようです。確認すると、回答をさらに追究できる内容ノードに分解します。'],
  ['创建 {count} 个节点', 'Create {count} nodes', '{count} 個のノードを作成'],
  ['已保留完整回答', 'Kept the complete answer', '回答全体を保持しました'],
  ['你可以稍后从回答下方手动拆解。', 'You can manually split it from the answer controls later.', '後で回答の操作から手動で分解できます。'],
  ['已自动拆解回答', 'Answer split automatically', '回答を自動分解しました'],
  ['已根据你的请求创建 {count} 个可继续追问的节点。', 'Created {count} nodes for follow-up exploration based on your request.', 'リクエストに応じて、さらに追究できるノードを {count} 個作成しました。'],
  ['自动拆解失败', 'Automatic decomposition failed', '自動分解に失敗'],
  ['内容模块 {count}', 'Content module {count}', '内容モジュール {count}'],
